...
- **Ollama** is running locally with the default model (e.g. `llama3.2:3b`). The gateway runs one agent turn per inbound message and sends the model's reply back to Telegram; if Ollama is unavailable or the model is missing, the message is received but no reply is sent (see verification below).
...
- **"inbound: agent turn failed"**  
  - Ollama is not running, or the default model (e.g. `llama3.2:3b`) is not available. Start Ollama and pull the model, or set `agents.defaultModel` in config to a model you have.
...
2. **In the logs:**
   - **No new log line after you send a message** — The message may not have reached the gateway yet (webhook/network), or the agent turn is still running (Ollama thinking or a tool still running). With `RUST_LOG=debug` you can see when the agent starts and when tools run.
   ...
   - **`agent: tool … failed: …`** — A tool failed. The model is given the error and may reply; if the turn later fails or the model returns nothing, you get no reply unless the gateway sends the generic error.

3. **After adding skills or tools:** If the model often uses tools, turns can take longer. If a tool hangs (e.g. a CLI not responding), the gateway will appear to "think" until it times out or errors. Use `RUST_LOG=debug` to see which tool ran and whether it failed.
