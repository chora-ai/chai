...
pub fn ui_agent_screen(app: &mut ChaiApp, ui: &mut egui::Ui, running: bool) {
    crate::app::ui_screen(
        ...
        |ui| {
            ...
            // Resolve context mode from gateway status.
            let is_read_on_demand = gs
                .agent_skills
                .get(&selected_id)
                    ...
                    if is_read_on_demand {
                        dashboard::dashboard_two_columns(ui, |ui_left, ui_right| {
                            ...
                            if let Some(bodies_map) = &status_bodies {
                                ...
                                // Gateway returned agent detail but no skills
                                // context — the agent has no on-demand skills.
                                ui_right.label("No skills context for this agent.");
                            }
                        });
                    } else {
                        if let Some(text) = context_text {
