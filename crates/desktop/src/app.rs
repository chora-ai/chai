...
impl ChaiApp {
    ...
    fn ui_info_screen(&mut self, ui: &mut egui::Ui, running: bool) {
        ...
        ui.columns(2, |columns| {
            ...
            // Right column: Context and (when read-on-demand) Skills
            {
                ...
                // Context: full mode = everything (date + agent + full skills); read-on-demand = upfront only (date + agent + skills overview)
                ...
                if let Some(text) = context_text {
                    ...
                    let context_height = if context_mode == Some("readOnDemand") {
                        (available * 0.4).max(40.0)
                    } else {
                        available
                    };
                    egui::ScrollArea::vertical()
                ...
                // Skills: only when read-on-demand (full mode shows everything in Context)
                if context_mode == Some("readOnDemand") {
                    ui_right.add_space(INFO_SUBSECTION_SPACING);
                    ui_right.label(egui::RichText::new("Skills (read-on-demand)").strong());
                    ui_right.add_space(INFO_LINE_SPACING);
                    // Prefer skill bodies only (no overview) so the overview appears only in Context; fallback to full with intro stripped for older gateways.
                    let skills_text = self
                        .gateway_status
                        .as_ref()
                        .and_then(|s| {
                            s.skills_context_bodies
                                .as_deref()
                                .filter(|t| !t.trim().is_empty())
                                .or_else(|| {
                                    s.skills_context_full.as_deref().or(s.skills_context.as_deref())
                                        .map(|s| {
                                            const FULL_INTRO: &str = "You have access to the following skills:\n\n";
                                            if s.starts_with(FULL_INTRO) {
                                                s[FULL_INTRO.len()..].trim_start()
                                            } else {
                                                s
                                            }
                                        })
