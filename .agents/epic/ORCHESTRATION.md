...
**Continuation** — Per-agent **workspace layout** (`~/.chai/agents/<agentId>/`), **per-agent skill policy** (moving enablement and context mode off a global **`skills`** block), and **role-correct** system prompts for workers are **not** part of phases 1–4. That work is specified in **[AGENT_ISOLATION.md](AGENT_ISOLATION.md)** and supersedes the current assumption that workers share the orchestrator’s full system context and global skill configuration.
