...
| **Clients** | Chai Desktop: chat timeline for delegation events; **Info** collapsible orchestration catalog. |
...
- Gateway **`status`** exposes **`orchestrationCatalog`** (merged discovery + allowlist-only rows); see [`catalog.rs`](../crates/lib/src/orchestration/catalog.rs) and **Chai Desktop** **Info**.
