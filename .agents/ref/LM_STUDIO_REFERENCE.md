...
- **Config** — LM Studio is selected when **`agents.defaultProvider`** is **`"lms"`** (see [EPIC_API_ALIGNMENT.md](../EPIC_API_ALIGNMENT.md) for the full provider list). **`agents.defaultModel`** is the model id passed as-is (e.g. `openai/gpt-oss-20b`, `ibm/granite-4-micro`). Under **`providers`** use **`lms`** for LM Studio settings. Only **`baseUrl`** is supported (default `http://127.0.0.1:1234/v1`).
