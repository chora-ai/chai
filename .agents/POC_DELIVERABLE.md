...
- **Skills** — Loader (bundled, workspace, extra), gating by `metadata.requires.bins`, two bundled skills: `notesmd-cli` and `obsidian` (each with `SKILL.md` + safe exec + tool layer). See [Skills and the LLM](POC_IMPLEMENTATION.md#skills-and-the-llm).
