...
- **Skills** — Added loader (config dir skills + extraDirs), gating by `metadata.requires.bins`; bundled skills `notesmd`, `notesmd-daily`, `obsidian`, and `obsidian-daily` with SKILL.md and tools.json; generic executor from descriptors; optional read-on-demand (`skills.contextMode`: compact list + `read_skill` tool); optional scripts (`scripts/` for param resolution via `resolveCommand.script`).
