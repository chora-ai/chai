...
A minimalist, privacy-preserving multi-agent management system designed for constrained-model operation.
...
- **Privacy by construction** — Sensitive data stays on the local machine, handled by local models. Skill authoring (which may use cloud APIs) operates only on non-sensitive structural artifacts. Runtime profiles enforce trust boundaries architecturally — the developer profile *cannot* access assistant workspace content regardless of model behavior.
...
- **Open-source** — MIT-licensed. Skills are portable, inspectable text files.
