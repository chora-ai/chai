...
**Status** — **Implemented** in **`crates/lib/src/channels/signal.rs`**: **`GET /api/v1/events`** (SSE) for JSON-RPC **`receive`** notifications with **`dataMessage.message`**; **`POST /api/v1/rpc`** with **`send`** (`recipient` +E.164 or **`groupId`**).
