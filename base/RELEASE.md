...
Each release has a branch named `release/vX.Y.x`, created from `main`. The `x` denotes the patch version slot — the branch exists for the lifetime of that minor version and is the target for all patch releases within it (e.g., `release/v0.1.x` receives v0.1.0, v0.1.1, v0.1.2, etc.).
...
7. **Commit and tag** — Commit on the release branch with message `vX.Y.P`, then create an annotated tag using the tag file with `--cleanup=verbatim` to preserve markdown formatting.
