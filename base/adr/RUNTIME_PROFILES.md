...
- **Switching profiles requires stopping the gateway for the target profile.** `chai profile switch` rewrites the symlink only when no gateway is running for the target profile (enforced via per-profile advisory exclusive locks on `~/.chai/profiles/<name>/gateway.lock`). Other profiles' gateways are unaffected. The desktop allows profile switching even while a gateway runs on a different profile.
...
- **Multiple simultaneous gateways.** Per-profile locks allow multiple gateways to run on different profiles at the same time. Profile switching only requires stopping the gateway on the target profile.
