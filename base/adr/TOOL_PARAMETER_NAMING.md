...
|------|-----------|---------|
| Target path | `path` — the file, note, or directory the tool operates on directly | `files_read` → `path`, `git_log` → `path` (file within repo) |
...
| Flag-name alignment | Use the exact long-form CLI flag name converted to `snake_case` | `--files-with-matches` → `files_with_matches`, `--ignore-case` → `ignore_case` |
