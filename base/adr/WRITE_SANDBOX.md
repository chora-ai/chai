...
- **Upward traversal by external commands is validated at the resolve-command layer.** Some CLI commands (git, cargo, etc.) traverse upward from the working directory to find project-root markers (`.git`, `Cargo.toml`). When a `workingDir` parameter points to a sandbox subdirectory that does not contain its own project root, the command can escape the sandbox boundary by finding a root in a parent directory. Bundled skills use the `chai resolve` subcommand for sandbox-aware path resolution; custom skills may use shell scripts via the `resolveCommand.script` mechanism. The resolve command first unconditionally validates that the working directory is inside the sandbox (regardless of whether project discovery succeeds), then validates that the resolved project root is inside the sandbox (including symlinked entries at the top level of the sandbox directory). The unconditional check prevents path-traversal attacks where a non-existent or non-project target directory bypasses validation when the discovery command fails. This validation is enforced by resolve-command error propagation: if a resolve command exits non-zero, the tool call is rejected — the command never runs. This defense is skill-specific (each skill's resolve command handles its own command's traversal behavior) rather than generic, because the discovery mechanism differs per command (`git rev-parse --git-dir` vs. `cargo locate-project`).
- **Resolve-command errors are propagated.** When a resolve command exits with a non-zero code, the executor rejects the tool call instead of silently falling back to the unresolved value. This makes resolve-command validation effective — a validation failure in a resolve command (e.g., detecting that git would operate on a repository outside the sandbox) prevents the command from running. Before this fix, resolve-command errors were silently swallowed and the raw parameter value was used, allowing tool calls to proceed with unvalidated paths.
