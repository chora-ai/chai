...
- **Upward traversal by external commands is validated at the resolve-script layer.** Some CLI commands (git, cargo, etc.) traverse upward from the working directory to find project-root markers (`.git`, `Cargo.toml`). When a `workingDir` parameter points to a sandbox subdirectory that does not contain its own project root, the command can escape the sandbox boundary by finding a root in a parent directory. Resolve scripts for affected skills validate that the command's resolved project root is inside the sandbox (including symlinked entries at the top level of the sandbox directory) before allowing the command to proceed. This validation is enforced by the resolve-script error propagation: if a resolve script exits non-zero, the tool call is rejected — the command never runs. This defense is skill-specific (each skill's resolve script handles its own command's traversal behavior) rather than generic, because the discovery mechanism differs per command (`git rev-parse --git-dir` vs. `cargo locate-project`).
- **Resolve-script errors are propagated.** When a resolve command exits with a non-zero code, the executor rejects the tool call instead of silently falling back to the unresolved value. This makes resolve-script validation effective — a validation failure in a resolve script (e.g., detecting that git would operate on a repository outside the sandbox) prevents the command from running. Before this fix, resolve-script errors were silently swallowed and the raw parameter value was used, allowing tool calls to proceed with unvalidated paths.
