...
- **Resolve script error propagation**: When a resolve command exits with a non-zero code, the executor rejects the tool call instead of silently falling back to the unresolved parameter value. This enables resolve scripts to perform validation — e.g., a resolve script can verify that git would find its repository inside the sandbox and exit non-zero if the repository root is outside, preventing the git command from running. Before this behavior, resolve-script errors were silently swallowed and the raw parameter value was used, allowing tool calls to proceed with unvalidated paths.
