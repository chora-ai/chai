...
**Status** — **Phased delivery for this epic is complete** (implementation phases **1–4** below). Delegation, policy, observability, desktop surfacing, merged **`agents.orchestrationCatalog`** on gateway **`status`**, and advanced orchestrator policy (allowlists, session caps, blocked providers, instruction routes) are implemented. **Optional follow-ups** (richer catalog metadata, analytics, CLI parity) and **out-of-scope** items (see **Scope**) are tracked separately—see **Follow-ups** and **Related Epics and Docs**.

**Continuation** — Per-agent **context directories**, **per-agent `skillsEnabled` / `contextMode`**, and **role-correct** worker prompts were specified in **[AGENT_ISOLATION.md](AGENT_ISOLATION.md)** and are **shipped**. Historical phase-3 notes below that described workers sharing the orchestrator’s system context are **outdated**; **current** behavior is in **[spec/ORCHESTRATION.md](../spec/ORCHESTRATION.md)** and **`execute_delegate_task`** (worker **`WorkerDelegateRuntime`**: that worker’s **`AGENT.md`**, skills, tools—no **`## Workers`**, no **`delegate_task`**).

**Delegation spec** — Behavior for **`delegate_task`** (built-in tool), workers, limits, and gateway surfaces is documented in **[spec/ORCHESTRATION.md](../spec/ORCHESTRATION.md)**. This epic file focuses on goals, config evolution, phases, requirements, and closure notes. Update **`spec/ORCHESTRATION.md`** when **AGENT_ISOLATION** changes worker turn context behavior.
...
| **Protocol** | WebSocket **`orchestration.delegate.*`** events; **`status` payload `agents.orchestrationCatalog`**; **`DelegateContext`** with **`DelegateObservability`** and session wiring for policy. |
| **Clients** | Chai Desktop: chat timeline for delegation events; **Status** collapsible orchestration catalog. |

## Problem Statement

The gateway runs a single default model for the entire agent loop, with no way to use different models for different steps. This means there is no mechanism to delegate narrow subtasks to smaller or faster models, route sensitive steps to local or self-hosted workers, or optimize cost and latency by matching model capability to step complexity.
...
- Gateway **`status`** payload **`agents.orchestrationCatalog`** (merged discovery + allowlist-only rows); see [`catalog.rs`](../../crates/lib/src/orchestration/catalog.rs) and **Chai Desktop** **Status**.
- Connection settings live under top-level **`providers`**; see **Configuration Model** below.

## Scope

### In Scope

Delegation primitive; orchestrator–worker loop with **`delegate_task`**; dispatch and config-driven **policy** (allowlists, caps, blocks, instruction routes); **merged catalog** on **`status`**; observability (logs + WebSocket events + desktop); worker invocation via gateway paths.

### Out of Scope

Adding **new provider backends** as product work—see [API_ALIGNMENT.md](API_ALIGNMENT.md). **Full sandboxing**, **arbitrary code exec approval**, and **interactive human approval queues** for delegation (pause/resume, pending state, multi-user review)—these belong in a **separate security/UX epic** if prioritized; **`delegateBlockedProviders`** covers "never delegate to this provider" without an approval workflow.

## Design

### Configuration Model (Today)

Before orchestration, config already separates **integration plumbing** from **agent configuration**:
...
| **`agents`** | **JSON array** of entries with **`id`**, **`role`** (`orchestrator` \| `worker`), per-entry defaults (`defaultProvider`, `defaultModel`, …), and per-entry **`skillsEnabled`** / **`contextMode`**. Exactly one orchestrator; **`agents`** may be omitted for a built-in default orchestrator (id `orchestrator`). See **Likely Evolution** (Option A). |

**Skill packages** are **not** a top-level config object: shared root **`~/.chai/skills`**, with **`skillsEnabled`** / **`contextMode`** on each **`agents`** entry — **[AGENT_ISOLATION.md](AGENT_ISOLATION.md)**, **[spec/CONFIGURATION.md](../spec/CONFIGURATION.md)**.

**Rationale:** **`agents`** holds *which* provider and model each role uses, discovery scope (**`enabledProviders`**), delegation policy, and *which* skill packages apply (**`skillsEnabled`**, **`contextMode`**). **`providers`** holds *how to reach* each stack (credentials, URLs).
...
- **Cons:** Two top-level shapes; harder to extend if more roles appear or multiple orchestrators are allowed.
...
- [x] **Orchestrator loop and tool semantics (phase 3 slice)** — `delegate_task` invokes a worker turn with chosen provider/model and instruction; worker uses **that worker’s** cached system context and per-worker skill tools (**[AGENT_ISOLATION.md](AGENT_ISOLATION.md)**); nested `delegate_task` disabled; structured JSON result merged into the orchestrator turn. Optional **multi-turn planning** is a **Follow-up**, not required for epic closure.
...
| **Model choice** | Defaults + overrides; **`delegateAllowedModels`**; **`delegationInstructionRoutes`**; **`agents.orchestrationCatalog`** on **`status`** | Richer per-model metadata (e.g. max context); automatic routing policies beyond config |
| **Privacy / routing** | **`delegateAllowedModels`**, **`delegateBlockedProviders`**, caps, **`local`** / **`toolCapable`** hints | Human-in-the-loop approval, sandboxed execution—see **Related Epics and Docs** |
...
- Worker delegation selects provider/model via **`resolve_delegate_target`** and **`ProviderClients::as_dyn`**; the main session still uses the orchestrator’s default unless overridden per request.
...
|--------|----------------|
| **Per-agent workspace and skills**, worker vs orchestrator context | [AGENT_ISOLATION.md](AGENT_ISOLATION.md) |
| **New model providers** and wire-protocol work | [API_ALIGNMENT.md](API_ALIGNMENT.md) |
...
| **Named runtime profiles** (trust boundaries, profile-local config) | [RUNTIME_PROFILES.md](RUNTIME_PROFILES.md) (complements per-agent isolation) |
