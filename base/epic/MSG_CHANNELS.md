...
- **Matrix** — **Shipped:** **`MatrixChannel`** (`lib`) over **`matrix_channel::MatrixInner`** with SQLite + **E2EE**; **room allowlist**; **gateway HTTP verification** (SAS) under **`/matrix/verification/*`**. Reference: [MATRIX.md](../ref/MATRIX.md); journey: [`docs/journey/08-channel-matrix.md`](../../docs/journey/08-channel-matrix.md).
